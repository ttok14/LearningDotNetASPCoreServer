// <auto-generated />
using System;
using LearningServer01;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace LearningServer01.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260303090030_HeroItemUIDToForeignKey")]
    partial class HeroItemUIDToForeignKey
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            MySqlModelBuilderExtensions.AutoIncrementColumns(modelBuilder);

            modelBuilder.Entity("LearningServer01.Data.DeploymentSlot", b =>
                {
                    b.Property<long>("SlotUID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<long>("SlotUID"));

                    b.Property<long?>("EquippedItemUID")
                        .HasColumnType("bigint");

                    b.Property<string>("OwnerID")
                        .IsRequired()
                        .HasColumnType("varchar(255)");

                    b.Property<int>("SlotIdx")
                        .HasColumnType("int");

                    b.HasKey("SlotUID");

                    b.HasIndex("EquippedItemUID");

                    b.HasIndex("OwnerID");

                    b.ToTable("DeploymentSlots");
                });

            modelBuilder.Entity("LearningServer01.Data.EntityItemInfo", b =>
                {
                    b.Property<long>("UID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<long>("UID"));

                    b.Property<int>("Level")
                        .HasColumnType("int");

                    b.Property<string>("OwnerID")
                        .IsRequired()
                        .HasColumnType("varchar(255)");

                    b.Property<float>("PositionX")
                        .HasColumnType("float");

                    b.Property<float>("PositionZ")
                        .HasColumnType("float");

                    b.Property<float>("RotationY")
                        .HasColumnType("float");

                    b.Property<string>("SpecificDataJson")
                        .HasColumnType("longtext");

                    b.Property<int>("TableID")
                        .HasColumnType("int");

                    b.HasKey("UID");

                    b.HasIndex("OwnerID");

                    b.ToTable("Entities");
                });

            modelBuilder.Entity("LearningServer01.PlayerInfo", b =>
                {
                    b.Property<string>("ID")
                        .HasColumnType("varchar(255)");

                    b.Property<int>("Bounty")
                        .HasColumnType("int");

                    b.Property<long?>("EquippedHeroItemUID")
                        .HasColumnType("bigint");

                    b.Property<int>("Food")
                        .HasColumnType("int");

                    b.Property<int>("Gold")
                        .HasColumnType("int");

                    b.Property<int>("Level")
                        .HasColumnType("int");

                    b.Property<string>("MapName")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Nickname")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<int>("SkillID01")
                        .HasColumnType("int");

                    b.Property<int>("SkillID02")
                        .HasColumnType("int");

                    b.Property<int>("SkillID03")
                        .HasColumnType("int");

                    b.Property<int>("SpellID01")
                        .HasColumnType("int");

                    b.Property<int>("SpellID02")
                        .HasColumnType("int");

                    b.Property<int>("SpellID03")
                        .HasColumnType("int");

                    b.Property<string>("StatusMsg")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<int>("StrengthStat")
                        .HasColumnType("int");

                    b.Property<int>("Wood")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("EquippedHeroItemUID");

                    b.ToTable("Players");
                });

            modelBuilder.Entity("LearningServer01.UserItem", b =>
                {
                    b.Property<long>("UID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<long>("UID"));

                    b.Property<int>("Level")
                        .HasColumnType("int");

                    b.Property<string>("OwnerID")
                        .IsRequired()
                        .HasColumnType("varchar(255)");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.Property<int>("TableID")
                        .HasColumnType("int");

                    b.HasKey("UID");

                    b.HasIndex("OwnerID");

                    b.ToTable("UserItems");
                });

            modelBuilder.Entity("LearningServer01.Data.DeploymentSlot", b =>
                {
                    b.HasOne("LearningServer01.UserItem", "EquippedItem")
                        .WithMany()
                        .HasForeignKey("EquippedItemUID");

                    b.HasOne("LearningServer01.PlayerInfo", "Owner")
                        .WithMany("DeploymentSlots")
                        .HasForeignKey("OwnerID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("EquippedItem");

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("LearningServer01.Data.EntityItemInfo", b =>
                {
                    b.HasOne("LearningServer01.PlayerInfo", "Owner")
                        .WithMany("PlacedEntities")
                        .HasForeignKey("OwnerID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("LearningServer01.PlayerInfo", b =>
                {
                    b.HasOne("LearningServer01.UserItem", "EquippedHeroItem")
                        .WithMany()
                        .HasForeignKey("EquippedHeroItemUID");

                    b.Navigation("EquippedHeroItem");
                });

            modelBuilder.Entity("LearningServer01.UserItem", b =>
                {
                    b.HasOne("LearningServer01.PlayerInfo", "Owner")
                        .WithMany("InventoryItems")
                        .HasForeignKey("OwnerID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("LearningServer01.PlayerInfo", b =>
                {
                    b.Navigation("DeploymentSlots");

                    b.Navigation("InventoryItems");

                    b.Navigation("PlacedEntities");
                });
#pragma warning restore 612, 618
        }
    }
}
